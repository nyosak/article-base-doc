...
`--slug` というオプションでファイル名指定可能なようだが、この名称には自分の記事だけでなく、Zenn の記事全体でユニークという無茶な制限があるので、自動投稿での指定は簡単ではないかも。使用済みだったらランダム生成に自動で切り替わってくれる方式なら良いのだが。
...
その一方で、今のランダム文字列だと、投稿が増えてくると、どの記事が base と紐付いているのか探すのが困難になってくると思われるので、やはり指定が必要か。
...
記事ファイルの更新は、article-base-doc の記事の文書情報から残りの項目を更新し、文書ファイルの中身を末尾に追加すれば良い。
...
投稿後に、 git pull してローカルを同期させておくのが良い。ただし、タスクの完了に時間がかかるので、投稿操作の直後ではなく、公開確認後だ。
...
このリンク先は自分がアップしたファイルのとおりなので、投稿前に決めておくことができ、原稿を書くには都合がよい。GitHub レポジトリの raw ファイル参照でも良いのだが、上記の方が URL としてはスマートだ。
